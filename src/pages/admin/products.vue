<template>
      ...
      <v-col cols="12">
        <v-data-table :items="products" :headers="headers" :search="search">
          ...
          <template #[`item.image`]="{ value }">
            <v-img :src="value" height="50" class="cursor-pointer" @click="openImageDialog(value)"></v-img>
          </template>
  ...
  <v-dialog v-model="dialog.open" persistent transition="dialog-bottom-transition">
  ...
  <v-dialog v-model="imageDialog.open" max-width="600">
    <v-card>
      <v-img :src="selectedImage" max-height="500" contain></v-img>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn text @click="closeImageDialog">{{ $t('adminProduct.cancel') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
const headers = computed(() => {
  return [
    ...
    // key 設定成一個不存在的欄位，所以沒有 value，但需要知道裡面的 item(資料)
